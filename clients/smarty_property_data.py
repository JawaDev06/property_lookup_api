...
def _garage_code_from_raw(value: Any) -> Optional[str]:
    """Return A for attached garage and D for detached garage when Smarty indicates it."""
    garage_type = _garage_type_from_raw(value)
    if garage_type == "Attached":
        return "A"
    if garage_type == "Detached":
        return "D"
    return None
...
def property_data_from_smarty_payload(payload: Any, source: str = "smarty_property_data") -> Optional[PropertyData]:
    ...
    data.garage_code = _garage_code_from_raw(data.garage)

    if all(getattr(data, field) is None for field in (
        "latest_assessed_value", "square_feet", "year_built", "beds", "baths", "last_sold_date",
        "last_sold_amount", "garage", "garage_summary", "garage_code", "garage_sqft", "parking_spaces",
