...
def _optional_extra_columns() -> dict[str, tuple[str, object, str, str]]:
    """v7 writes one garage column at R by default and no U:AD data columns."""
    return {
...
def _remove_columns_u_ad(ws) -> None:
    """Delete columns U:AD by default to remove sale/extra Smarty output columns.

    Set SMARTY_DELETE_COLUMNS_U_AD=false to leave the worksheet layout untouched.
    """
    if not _env_bool("SMARTY_DELETE_COLUMNS_U_AD", True):
        return
    # U = 21 and AD = 30, so delete 10 columns.
    ws.delete_cols(21, 10)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fill weekly workbook property columns using Smarty US Property Data only. v8 writes garage to R, removes U:AD, and does not add formatting/comments.")
