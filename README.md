# Smarty-only workbook tool v8

This version uses Smarty only. It writes the original core fields and changes garage output:
...
- **P** beds
- **Q** baths
- **R** garage value, formatted as `A 420`, `D 240`, `A`, `D`, or just sqft when only sqft is available
- Deletes/removes columns **U:AD** by default after processing

`A` = attached garage, `D` = detached garage. Smarty only returns A/D when its property data includes enough garage detail to identify attached vs detached.

Set `SMARTY_DELETE_COLUMNS_U_AD=false` in Railway Variables if you do not want the app to delete those columns.
...
## v8 formatting/comment behavior

This version does not add green cell fill/backgrounds and does not add cell comments for updated, not-found, or error rows. It only writes the requested cell values and number formats.
